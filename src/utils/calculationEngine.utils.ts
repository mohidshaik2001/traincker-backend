...
import { Biometrics, type IBiometrics } from "../models/biometrics.models.js";
...
import type { IFood } from "../models/food.models.js";
import type { IExercise } from "../models/exercise.models.js";
import type { ISupplement } from "../models/supplement.models.js";
...
type generatePlan = (
  ...
  dietPreference: IDietPreference,
  reverseEngineeredMetrics: reverseEngineeredMetrics,
) => Promise<IPlan | null>;

type processBiometrics = (biometrics: IBiometrics) => Promise<IBiometrics>;
export const generatePlan: generatePlan = async (
  biometrics,
  dietPreference,
  reverseEngineeredMetrics,
) => {
  try {
    const trainingArchitecture = determineTrainingArchitecture(
      biometrics.baseLineCapacity,
    );
    ...
    const targetMacros = calculateTargetmacros(
      biometrics,
      reverseEngineeredMetrics.weight,
      trainingArchitecture.weightsDays,
    );
...
export const processBiometrics: processBiometrics = async (biometrics) => {
  try {
    const newBiometrics: IBiometrics = biometrics;

    newBiometrics.currentFatPercentage = calculateFatPercentage(
      biometrics.gender,
      biometrics.measurements.weight,
      biometrics.measurements.height,
      biometrics.measurements.neck,
      biometrics.measurements.waist,
      biometrics.measurements.hips,
    );

    newBiometrics.currentTdee = calculateTdee(
      biometrics.measurements.weight,
      biometrics.measurements.height,
      biometrics.age,
      biometrics.gender,
...
export const calculateFatPercentage = (
  ...
  if (normalizedGender === "male" || normalizedGender === "m") {
    if (waist <= neck) return 0; // Prevent NaN on log10 of negative or zero values
    const densityMale =
      1.0324 -
    ...
    if (waist + femaleHips <= neck) return 0;
    const densityFemale =
      1.29579 -
...
export const determineTrainingArchitecture = (
  baseLineCapacity: IBiometrics["baseLineCapacity"],
): ITrainingArchitecture => {
...
export const calculateTargetmacros = (
  ...
  if (
    biometrics.gender.toLowerCase() === "male" ||
  ...
  if (trainingDays === 3 || trainingDays === 4) activityMultiplier = 1.375;
...
export const determineMealSchedule = (targetCalories: number): string[] => {
...
export const buildDietPlan = async (
  targetMacros: any,
  dietPreference: any,
): Promise<ICustomizedDietPlan | null> => {
  try {
    const mealNames = determineMealSchedule(targetMacros.calories);
    const targetMealsPerDay = mealNames.length;

    const allowedFoodIds = dietPreference.acceptedFoods.map(
      (af: any) => af.foodId,
    );
    ...
    if (!availableFoods || !availableFoods.length) {
      throw new ApiError(
        400,
        [],
        "No accepted foods found for this diet preference.",
      );
    }
    ...
    availableFoods.forEach((food: any) => {
    ...
    const mealsMap: Record<
      string,
      { foodId: Types.ObjectId; quantity: number }[]
    > = {};
    ...
    mealNames.forEach((mealName, index) => {
      ...
      const pFood =
        proteinSources.length > 0
          ...
          : null;
      const cFood =
        carbSources.length > 0 ? carbSources[index % carbSources.length] : null;
      const fFood =
        fatSources.length > 0 ? fatSources[index % fatSources.length] : null;

      const mealItems = [];

      if (pFood) {
        const qty = Math.round(
          (mealProtein / Math.max(1, pFood.protein)) * pFood.baseServingAmount,
        );
        mealItems.push({ foodId: pFood._id, quantity: qty });
      }
      if (cFood) {
        const qty = Math.round(
          (mealCarbs / Math.max(1, cFood.carbs)) * cFood.baseServingAmount,
        );
        mealItems.push({ foodId: cFood._id, quantity: qty });
      }
      if (fFood) {
        const qty = Math.round(
          (mealFat / Math.max(1, fFood.fats)) * fFood.baseServingAmount,
        );
...
export const buildTrainingPlan = async (
  architecture: ITrainingArchitecture,
): Promise<any> => {
  try {
    ...
    const buckets = {
      upperCompound: allExercises.filter(
        (e: any) =>
          e.mechanic === "Compound" &&
          ["Chest", "Back", "Shoulders"].includes(e.targetMuscle),
      ),
      upperIsolation: allExercises.filter(
        (e: any) =>
          e.mechanic === "Isolation" &&
          ["Chest", "Back", "Shoulders", "Arms"].includes(e.targetMuscle),
      ),
      lowerCompound: allExercises.filter(
        (e: any) => e.mechanic === "Compound" && e.targetMuscle === "Legs",
      ),
      lowerIsolation: allExercises.filter(
        (e: any) =>
          e.mechanic === "Isolation" &&
          ["Legs", "Calves"].includes(e.targetMuscle),
      ),
      fullBody: allExercises.filter((e: any) => e.mechanic === "Compound"),
      cardio: allExercises.filter(
        (e: any) => e.mechanic === "Cardio" || e.type === "Cardio",
      ),
    };
    ...
    const scheduleMap: Record<
      string,
      { exerciseId: Types.ObjectId; sets: number; reps: number }[]
    > = {};

    // Helper function to safely get an exercise and rotate the bucket
    const getExercise = (bucket: any[]) => {
...
export const buildSupplementPlan = async (
  dietPreference: any,
): Promise<any> => {
  try {
    ...
    const scheduleMap: Record<
      string,
      { supplementId: Types.ObjectId; quantity: number; time: string }[]
    > = {};

    // Helper function to safely find a supplement by keyword in its name
    const findSupp = (keyword: string) => {
      return allSupplements.find(
        (s: any) =>
    ...
    const hasDairyAllergy = dietPreference.allergies?.some(
      (a: string) =>
        a.toLowerCase().includes("dairy") || a.toLowerCase().includes("milk"),
    );
