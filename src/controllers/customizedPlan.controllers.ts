...
export const getCustomizedPlan = asyncHandler(async function (req, res) {
  try {
    ...
    const [dietPlan, trainingPlan, supplementPlan] = await Promise.all([
      CustomizedDietPlan.findById(customizedDietPlanId).populate(
        "meals.$*.foodId",
        "name calories protein carbs fats servingAmount servingUnit",
      ),
      CustomizedTrainingPlan.findById(customizedTrainingPlanId).populate(
        "schedule.$*.exerciseId",
        "name targetMuscle mechanic",
      ),
      CustomizedSupplementPlan.findById(customizedSupplementPlanId).populate(
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          { dietPlan, trainingPlan, supplementPlan, calculatedTargets },
          "customized plan found",
        ),
      );
  } catch (error) {
