...
import {
  DietPreference,
  type IDietPreference,
} from "../models/dietPreference.models.js";
import {
  generatePlan,
  processBiometrics,
} from "../utils/calculationEngine.utils.js";
...
export const setUserAssessment = asyncHandler(async function (req, res) {
  try {
    ...
    if (userAssessment) {
      const biometricData: IBiometrics = await processBiometrics(biometrics);
      await Biometrics.findByIdAndUpdate(
      ...
      await DietPreference.findByIdAndUpdate(
      ...
      res
        ...
        .json(
          new ApiResponse(
            200,
            { userAssessment: userAssessment, biometrics: biometricData },
    ...
    const result = await UserAssessment.assembleAssessmentData(
      userId,
      biometricData,
      dietPreferences,
    );
    ...
    res
      .status(201)
      .json(
        new ApiResponse(201, final_response, "assessment created successfully"),
      );
  } catch (error) {
...
export const buildCustomizedPlan = asyncHandler(async function (req, res) {
  try {
    ...
    if (userAssessment.isAssessmentCompleted) {
      ...
      userAssessment.customizedPlanId = undefined as any;
      await userAssessment.save();
    }

    const customizedPlan = await CustomizedPlan.buildTargetProtocols(
      assessmentId,
      biometricsId,
      dietPreferenceId,
      reverseEngineeredMetrics,
